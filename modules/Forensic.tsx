  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div className="mb-6">
        ...
        <p className="text-lg text-slate-600 mt-2">Detección, prevención e investigación de fraude y corrupción.</p>
      </div>

      {/* Visual Fraud Triangle */}
      <Card title="El Triángulo del Fraude (Cressey)" className="bg-slate-50 border-slate-200">
        ...
        <div className="relative h-64 w-full max-w-md mx-auto">
             ...
             <div className="absolute top-0 left-1/2 -translate-x-1/2 w-40 flex flex-col items-center animate-slide-up" style={{ animationDelay: '0.1s' }}>
                <div className="w-16 h-16 bg-red-100 text-red-600 rounded-full flex items-center justify-center mb-2 shadow-sm border-2 border-red-200">
                    <AlertTriangle size={32} />
                </div>
                <div className="text-center">
                    <p className="font-bold text-red-800">Presión</p>
                    <p className="text-xs text-slate-500 leading-tight">Problemas financieros, vicios, metas irreales.</p>
                </div>
             </div>

             <div className="absolute bottom-0 left-0 w-36 flex flex-col items-center animate-slide-up" style={{ animationDelay: '0.3s' }}>
                <div className="w-16 h-16 bg-amber-100 text-amber-600 rounded-full flex items-center justify-center mb-2 shadow-sm border-2 border-amber-200">
                    <Lock size={32} />
                </div>
                <div className="text-center">
                    <p className="font-bold text-amber-800">Oportunidad</p>
                    <p className="text-xs text-slate-500 leading-tight">Ausencia de controles, supervisión deficiente.</p>
                </div>
             </div>

             <div className="absolute bottom-0 right-0 w-36 flex flex-col items-center animate-slide-up" style={{ animationDelay: '0.5s' }}>
                <div className="w-16 h-16 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center mb-2 shadow-sm border-2 border-blue-200">
                    <BrainIcon />
                </div>
                <div className="text-center">
                    <p className="font-bold text-blue-800">Racionalización</p>
                    <p className="text-xs text-slate-500 leading-tight">"Me lo deben", "Es temporal", "Todos lo hacen".</p>
                </div>
             </div>

             {/* Connecting Lines (SVG) */}
             <svg className="absolute inset-0 pointer-events-none -z-10 text-slate-300" width="100%" height="100%">
      ...
      <div className="bg-white rounded-2xl border-l-4 border-l-brand-500 shadow-sm p-6">
        <div className="flex items-center gap-3 mb-4">
            <BookOpen className="text-brand-600" size={24} />
            <h3 className="font-bold text-lg text-slate-800">Estudio de Caso: "La Compra Fantasma"</h3>
        </div>
        ...
        <p className="font-bold text-slate-800 text-sm mb-2">¿Cómo podría haber prevenido esto la OCI?</p>
        
        {!showCaseResult ? (
            <button 
                onClick={() => setShowCaseResult(true)}
                className="text-sm bg-brand-50 text-brand-700 font-bold px-4 py-2 rounded-lg hover:bg-brand-100 transition-colors"
            >
